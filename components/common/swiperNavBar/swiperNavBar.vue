<template>
	...
	<scroll-view scroll-x='false' scroll-with-animation 
		...
		<view class="swiperTab" v-for="(item,index) in swiperTabList" :key='index' 
			:style="{ width:currentSwiperWidth,lineHeight:currentSwiperHeight }" 
...
<script>
	export default{
		...
		props:{
			...
			currentSwiperWidth:{ type:String },//当前导航的宽度 % rpx rpx px
			currentSwiperHeight:{ 
				type:String ,
				default: "88rpx"
			},//当前导航的高度度 rpx px
			currentSwiperLineShow:{ //是否显示线条
				type:Boolean ,
				default: true
			...
			currentSwiperLineAnimatie:{ //当前选中的导航栏线条过渡效果
				type:Boolean ,
				default: true
			},
		},
...
<style>
	...
	.swiperLine{ position: absolute;bottom: 3rpx;left: 0;width: 100%;z-index: 10; }
	.swiperLineActive{ margin: 0 auto;border-radius: 30rpx; }

	            ...
	            <view class="zb-table-tbody">
	              <view  :class="['item-tr',highlight&&isHighlight(item,index)?'current-row':'']"
	                ...
	                <view
	                    ...
	                    v-for="(ite,i) in transColumns" :key="i" @click="cellClick(item,index,i)">
            ...
            <view class="zb-table-tbody">
              <view
                ...
                <view class='item-td'  
                      ...
                      :key="index" @click="cellClick(ite,i,index)"
                      v-for="(item,index) in fixedLeftColumns">
				<template  v-if="item.type==='operation'">
				  <view style="display: flex;align-items: center;height: 100%">
				    <view
				        v-for="ren,ind in permission(ite,item.renders,index)"
				        :key="ind"
				        @click.stop="$emit(ren.func,ite,index)"
				        :style="{
				            display:'flex',
				            alignItems: 'center',
				            marginRight:item.renders.length>1?'8px':'0'
				          }">
				      <template v-if="ren.type==='custom'">
				        <view :class="ren.class||''" style="cursor: pointer">
				          {{ren.name}}
                  ...
                  <template v-else>
					  {{ item.filters?itemFilter(ite,item):formatterAction(ite,item,index,i) }}
                  </template>
                </view>
              </view>
            </view>
              ...
              <view  :class="['item-tr',highlight&&isHighlight(item,index)?'current-row':'']"
                ...
                <view
                    :style="[{
                      ...
                      borderRight:`${border?'1px solid #e8e8e8':''}`,
                      textAlign:ite.align||'left',
                    },getCellStyle(item,ite,index,i)]" @click="cellClick(item,index,i)"
...
export default {
    ...
    getCellStyle() {
     return (row, column, rowIndex, columnIndex)=>{
       ...
       if(typeof cellStyle==='function'){
         ...
         let cst= cellStyle({row, column, rowIndex, columnIndex})
  ...
  methods: {
    ...
    rowClick(row,index){
      this.globalClick('row',row,index,null)
    },
	cellClick(row,index,columnIndex){
	  this.globalClick('cell',row,index,columnIndex)
	},
	globalClick(type,row,index,columnIndex){
		if(this.highlight){
		...
		if(type=='cell'){
			this.$emit('cellClick',row,index,columnIndex)
		}else{
...
.item-th{
  ...
  overflow-y:hidden;               /* 内容超出宽度时隐藏超出部分的内容 */
  overflow-x: auto;

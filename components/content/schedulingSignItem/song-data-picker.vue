			...
			<view class="dialog-caption" v-if="openSearch">
				<view class="flex-body" style="width: 90%; ">
					<uni-easyinput
						...
						@confirm="searchClick"
					></uni-easyinput>
					<button type="default" @click="clear" style="margin-left: 24rpx;height:80%;">清除选中</button>
				</view>
			</view>
	...
	methods: {
		...
		searchClick() {
			...
			this.searchResult = this.searchDatas.filter(x =>{
				if(x.text==undefined){return false;}
				return x.text
					.split(' ')
					.join('')
		...
		clear() {
			let nothingObj=[{text:'请选择','value':''}]
			this.hide();
			this.inputSelected = [];console.log(this.loading);//nothingObj;
			this._dispatchEvent(nothingObj);
		...
		_processReadonly(dataList, valueArray) {
			...
			let eachArray=[];
			if (Array.isArray(valueArray)){
				eachArray=valueArray;
			}else if(valueArray){
				eachArray.push(valueArray);
			}
			for (let i = 0; i < eachArray.length; i++) {
				var value = eachArray[i];

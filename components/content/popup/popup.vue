<template>
	<view v-show="ifshow">
		<view  @tap="ableClose" @touchmove.stop.prevent class="popup-layer" >
...
<script>
	export default {
		...
		computed: {
			...
			_location() {
				const positionValue = {
					'top': `bottom:${this.bottom};width:100%;`,
					'bottom': 'top:0px;width:100%;',
					'left': 'right:0px;height:100%;',
					'right': 'left:0px;height:100%;',
				};
				if(this.popupHeight.length) {  //有传弹窗高度
					return `${positionValue[this.direction]} ${this._translate} height:${this.popupHeight};`
				}	
